...
namespace SampleBrowser.SfPdfViewer
{
    ...
    public partial class PDFViewerCustomToolbar_Phone : SampleView
    {
        ...
        string currentDocument = "Export-Import-Form-Data-Updated";
        ...
        Stream fdfStreamAnnotationsData;
        Stream fdfStreamFormData;
        ...
        {            
            ...
            var m_pdfDocumentStream = typeof(App).GetTypeInfo().Assembly.GetManifestResourceStream(filePath + "Assets." + "Export-Import-Annotations-Reviewed" + ".pdf");
            pdfViewerControl.LoadDocument(m_pdfDocumentStream);
            pdfViewerControl.GoToPage(2);
            parentGrid = mainGrid;
        ...
        private void Exportannotation_Clicked(object sender, EventArgs e)
        {
        }

        private void ImportAnnotation_Clicked(object sender, EventArgs e)
        {
            ExportAnnotation.IsVisible = true;
            ImportAnnotation.IsVisible = false;
            //pdfViewer.ImportFormData(fdfStreamFormData, DataFormat.Fdf);
            //DependencyService.Get<IAlertView>().Toast("Imported the form data successfully in the new/updated document.");

            pdfViewer.ImportAnnotations(fdfStreamAnnotationsData, AnnotationDataFormat.Fdf);
            DependencyService.Get<IAlertView>().Toast("Imported the annotations successfully in the original document.");
        }

        private void ExportAnnotation_Clicked_1(object sender, EventArgs e)
        {
            DependencyService.Get<IAlertView>().Toast("Exported the annotations from the reviewed document.");
            ImportAnnotation.IsVisible = true;
            ExportAnnotation.IsVisible = false;
            //if (fdfStreamFormData != null)
            //    fdfStreamFormData = null;
            //fdfStreamFormData = pdfViewer.ExportFormData(DataFormat.Fdf, "FormFillingDocument");
            //Stream stream = typeof(App).GetTypeInfo().Assembly.GetManifestResourceStream("SampleBrowser.SfPdfViewer.Assets.Export-Import-Form-Data-Updated.pdf");
            //pdfViewer.LoadDocument(stream);
            //DependencyService.Get<IAlertView>().Toast("Exported the form data from the older document.");

            if (fdfStreamAnnotationsData != null)
                fdfStreamAnnotationsData = null;
            fdfStreamAnnotationsData = pdfViewer.ExportAnnotations(AnnotationDataFormat.Fdf);
            Stream stream = typeof(App).GetTypeInfo().Assembly.GetManifestResourceStream("SampleBrowser.SfPdfViewer.Assets.Export-Import-Annotations-Original.pdf");
            pdfViewer.LoadDocument(stream);
